...
class _InformationScreenContent extends StatelessWidget {
                ...
                Expanded(
                  child: SingleChildScrollView(
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          _buildTextField(context, '이름', state.name, '맹승주'),
                          _buildTextField(context, '성별', state.gender, '남성'),
                          _buildTextField(context, '나이', state.age, '25'),
                          _buildTextField(context, '발달장애 진단명', state.diagnosis, '자폐성장애'),
                          _buildTextField(context, '현재 복용 중인 약물', state.medication, '없음'),
                          _buildTextField(context, '주로 발생하는 문제 행동', state.behavioralIssues, '폭력적인 행동'),
                          _buildTextField(context, '행동 패턴 및 트리거 요인', state.behaviorPatterns, '스트레스'),
                          _buildTextField(context, '일상 생활 패턴', state.dailyRoutine, '일과'),
                        ],
                      ),
                    ),
                  ),
  ...
  Widget _buildTextField(BuildContext context, String name, String value, String initialValue) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
